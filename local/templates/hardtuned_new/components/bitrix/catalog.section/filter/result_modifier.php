...
/** @var array $arResult */
global $FILTER_LOST;
if($APPLICATION->GetCurPage() == '/cat/' && empty($FILTER_LOST['CAR']['ID'])) localRedirect('/catalog/');
...
if (!empty($arResult['ITEMS']))
{
    ...
    array("name" => "watermark", "position" => "center", "size"=>"real", "file"=>$_SERVER['DOCUMENT_ROOT']."/bitrix/modules/itturbo.printform/logo-bg.png")
		...
		if(!empty($arItem['PREVIEW_PICTURE']['ID'])) {
			$file = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width'=>250, 'height'=>250), BX_RESIZE_IMAGE_EXACT, true, $arWaterMark);
...
if(!empty($FILTER_LOST['CAR']['ID'])) {
	$arFilter = array();
	if(empty($FILTER_LOST['PAGE_CODE'])) $arFilter = Array('IBLOCK_ID' => 6, 'SECTION_ID' => $FILTER_LOST['CAR']['ID']);
	else {
		$res = CIBlockSection::GetList(Array("SORT"=>"ASC"), array('=CODE' => $FILTER_LOST['PAGE_CODE']));
		if($sections = $res->GetNext()) {
			$arFilter = Array('IBLOCK_ID' => 6, 'SECTION_ID' => $sections['ID']);
		}
	}
	if(!empty($arFilter)) {
		$res = CIBlockSection::GetList(Array("SORT"=>"ASC"), $arFilter, true, array());
		while($sections = $res->GetNext()) {
...
if(!empty($arResult['ITEMS'])) {
	...
	foreach ($arItem as $key => $arr) {
		foreach ($arr as $k => $item) {
			...
			if($ar_res = $res->GetNext()) {
				$arResult['COUNT'][$ar_res['NAME']] = array(
					'CODE' 		=> 	$ar_res['CODE'],
					'COUNT'		=>	CIBlockSection::GetSectionElementsCount($key)
				);
