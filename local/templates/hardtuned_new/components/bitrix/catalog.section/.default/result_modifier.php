...
if (!empty($arResult['ITEMS']))
{
    ...
    array("name" => "watermark", "position" => "center", "size"=>"real", "file"=>$_SERVER['DOCUMENT_ROOT']."/bitrix/modules/itturbo.printform/logo-bg.png")
		...
		if(!empty($arItem['PREVIEW_PICTURE']['ID'])) {
			$file = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width'=>250, 'height'=>200), BX_RESIZE_IMAGE_EXACT, true, $arWaterMark);
			$arItem['PREVIEW_PICTURE']['SRC'] = $file['src'];
		}
		if(!empty($arItem['PROPERTIES']['PROIZVODITEL']['VALUE'])) {
			$res = CIBlockElement::GetList( Array("SORT"=>"ASC"), Array('IBLOCK_ID' => 21, '=NAME' => $arItem['PROPERTIES']['PROIZVODITEL']['VALUE']));
			$arItem['PROPERTIES']['PROIZVODITEL']['VALUE'] = '';
			if($ar_res = $res->GetNext()) {
				if(!empty($ar_res['PREVIEW_PICTURE']))
					$arItem['PROPERTIES']['PROIZVODITEL']['VALUE'] = CFile::GetPath($ar_res['PREVIEW_PICTURE']);
			}
		}
...
if(!empty($arResult['ITEMS'])) {
	...
	foreach ($arResult['ITEMS'] as $key => $item) {
		...
		if($item["PROPERTIES"]["SELLOUT"]["VALUE"] == 'Да') {
			if(count($item["DISCOUNT"]) > 0 && !is_int(count($item["DISCOUNT"]) / 2) ) {
				$item["DISCOUNT"][] = [
		...
		if($item["PROPERTIES"]["KIT"]["VALUE"] == 'Да') {
			if(count($item["DISCOUNT"]) > 0 && !is_int(count($item["DISCOUNT"]) / 2)) {
				$item["DISCOUNT"][] = [
		...
		if($item["PROPERTIES"]["ORIGINAL"]["VALUE"] == 'Да') {
			if(count($item["DISCOUNT"]) > 0 && !is_int(count($item["DISCOUNT"]) / 2)) {
				$item["DISCOUNT"][] = [
		...
		if($item["PROPERTIES"]["HIGHTQUALITY"]["VALUE"] == 'Да') {
			$item["DISCOUNT"][] = [
		...
		$sec = CIBlockSection::GetList(Array(), Array("ID" => $item["IBLOCK_SECTION_ID"]), false, Array("NAME"));
		...
		//         $arWaterMark
		//     );
		//     $item['PREVIEW_PICTURE']['SRC'] = $arResizeFile['src'];
		// }
		$arItem[$item['IBLOCK_SECTION_ID']][] = $item;
	}
	foreach ($arItem as $key => $arr) {
		foreach ($arr as $k => $item) {
			$res = CIBlockSection::GetByID($key);
			if($ar_res = $res->GetNext())
				$arItemComplete[$ar_res['NAME']][] = $item;
		}
	}
	ksort($arItemComplete);
	$arResult['ITEMS'] = $arItemComplete;
}
?>