	...
	function PrintPropsForm($arSource = array(), $locationTemplate = ".default")
	{
		if (!empty($arSource))
		{
			...
			<div>
				...
				{
					if($arProperties['CODE'] == 'ARTICUL') continue;
					?>
					<div class="input<?if ($arProperties["REQUIED_FORMATED"]=="Y"):?> require-r<?endif?>" data-property-id-row="<?=intval(intval($arProperties["ID"]))?>">
						...
						elseif ($arProperties["TYPE"] == "TEXT")
						{
							?>
							<div class="col-md-8">
								<input type="text" maxlength="250" size="<?=$arProperties["SIZE1"]?>" value="<?=($arProperties["VALUE"] != $arProperties["DESCRIPTION"])?$arProperties["VALUE"]:'';?>" name="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["DESCRIPTION"]?>" id="<?=$arProperties["FIELD_NAME"]?>" />
							</div>
							<?
						}
						...
						elseif ($arProperties["TYPE"] == "TEXTAREA")
						{
							...
							<?if($arProperties["FIELD_NAME"] == 'ORDER_PROP_9'):
							global $APPLICATION;
									$APPLICATION->AddViewContent('comment_property', '<textarea rows="'.$rows.'" cols="'.$arProperties["SIZE1"].'" name="'.$arProperties["FIELD_NAME"].'" id="'.$arProperties["FIELD_NAME"].'" placeholder="'.$arProperties["DESCRIPTION"].'">'.(($arProperties["VALUE"] != $arProperties["DESCRIPTION"])?$arProperties["VALUE"]:'').'</textarea>');
								else:?>
									<div class="col-md-8">
									<textarea rows="<?=$rows?>" cols="<?=$arProperties["SIZE1"]?>" name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" placeholder="<?=$arProperties["DESCRIPTION"]?>"><?=($arProperties["VALUE"] != $arProperties["DESCRIPTION"])?$arProperties["VALUE"]:'';?></textarea>
								</div>
								<?endif;?>
