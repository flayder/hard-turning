...
foreach ($arResult['SECTIONS'] as $key => $arSection) {
	if(!empty($arSection['PICTURE']['ID'])) {
		//$file = CFile::ResizeImageGet($arSection['DETAIL_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT);
		$arResult['SECTIONS'][$key]['PICTURE'] = CFile::GetPath(['PICTURE']['ID']);
		print_r($arResult['SECTIONS'][$key]['PICTURE']);
	}
}
?>