...
global $USER;
// $APPLICATION->SetPageProperty("keywords", "шаблоны, битрикс");
// $APPLICATION->SetPageProperty("description", "супер шаблон битрикс");
...
if(!empty($arResult['PROPERTIES']['ORIGINAL_DETAIL']['VALUE']) && is_array($arResult['PROPERTIES']['ORIGINAL_DETAIL']['VALUE'])) {
		foreach ($arResult['PROPERTIES']['ORIGINAL_DETAIL']['VALUE'] as $key => $value) {
			$res = CIBlockElement::GetByID($value);
			if($ar_res = $res->GetNext()) {
			  	$price = CPrice::GetBasePrice($ar_res['ID']);
			  	$ar_res['PRICE_INFO'] = $price;
			  	$file = CFile::ResizeImageGet($ar_res['PREVIEW_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT);
		  		$ar_res['PREVIEW_PICTURE'] = $file['src'];
		  		$arResult['PROPERTIES']['ORIGINAL_DETAIL']['FULL_INFO'][] = $ar_res;
			}
		}
	} elseif(!empty($arResult['PROPERTIES']['ORIGINAL_DETAIL']['VALUE'])) {
		$res = CIBlockElement::GetByID($arResult['PROPERTIES']['ORIGINAL_DETAIL']['VALUE']);
		if($ar_res = $res->GetNext()) {
		  	$price = CPrice::GetBasePrice($ar_res['ID']);
		  	$ar_res['PRICE_INFO'] = $price;
		  	$file = CFile::ResizeImageGet($ar_res['PREVIEW_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT);
	...
	if(!empty($arResult['PROPERTIES']['MODIFICATIONS']['VALUE']) && is_array($arResult['PROPERTIES']['MODIFICATIONS']['VALUE'])) {
		foreach ($arResult['PROPERTIES']['MODIFICATIONS']['VALUE'] as $key => $value) {
			$res = CIBlockElement::GetByID($value);
			if($ar_res = $res->GetNext()) {
			  	$price = CPrice::GetBasePrice($ar_res['ID']);
			  	$ar_res['PRICE_INFO'] = $price;
		  		$arResult['PROPERTIES']['MODIFICATIONS']['FULL_INFO'][] = $ar_res;
			}
		}
	}

if(!empty($arResult['PROPERTIES']['POKUPAUT']['VALUE'])) {
	foreach ($arResult['PROPERTIES']['POKUPAUT']['VALUE'] as $key => $value) {
		$res = CIBlockElement::GetByID($value);
		if($ar_res = $res->GetNext()) {
		  	$price = CPrice::GetBasePrice($ar_res['ID']);
		  	$ar_res['PRICE_INFO'] = $price;
		  	$file = CFile::ResizeImageGet($ar_res['PREVIEW_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT);
		  	$ar_res['PREVIEW_PICTURE'] = $file['src'];
		  	$arResult['PROPERTIES']['POKUPAUT']['FULL_INFO'][] = $ar_res;
		}
	}
}

if(!empty($arResult['PROPERTIES']['sostav_komplekta']['VALUE'])) {
	foreach ($arResult['PROPERTIES']['sostav_komplekta']['VALUE'] as $key => $value) {
		$res = CIBlockElement::GetByID($value);
		if($ar_res = $res->GetNext()) {
		  	$price = CPrice::GetBasePrice($ar_res['ID']);
		  	$ar_res['PRICE_INFO'] = $price;
		  	$file = CFile::ResizeImageGet($ar_res['PREVIEW_PICTURE'], array('width'=>150, 'height'=>150), BX_RESIZE_IMAGE_EXACT);
		  	$ar_res['PREVIEW_PICTURE'] = $file['src'];
		  	$arResult['PROPERTIES']['sostav_komplekta']['FULL_INFO'][] = $ar_res;
		}
	}
}

if(!empty($arResult['ORIGINAL_PARAMETERS']['PROPERTY_CODE'])) {
	$arResult['DISPLAY_PROPERTIES'] = array();

	foreach ($arResult['ORIGINAL_PARAMETERS']['PROPERTY_CODE'] as $key => $pr) {
		if(isset($arResult['PROPERTIES'][$pr])) {
			$arResult['DISPLAY_PROPERTIES'][$pr] = $arResult['PROPERTIES'][$pr];
		}
	}
}

if(count($arResult['PROPERTIES']['MAGNITOLA_COMPLECT']['VALUE']) > 0) {
	foreach ($arResult['PROPERTIES']['MAGNITOLA_COMPLECT']['VALUE'] as $key => $value) {
		$res = CIBlockElement::GetByID($value);
		if($ar_res = $res->GetNext()) {
		  	$price = CPrice::GetBasePrice($ar_res['ID']);

		  	$VALUES = array();
   			$res = CIBlockElement::GetProperty($ar_res['IBLOCK_ID'], $ar_res['ID'], "sort", "asc", array("CODE" => "COMPLECTATION"));
   			...
   			$ar_res['PROPERTIES']['COMPLECTATION']['VALUE'] = $VALUES;

		  	$ar_res['PRICE_INFO'] = $price;
		  	$arResult['PROPERTIES']['MAGNITOLA_COMPLECT']['FULL_INFO'][] = $ar_res;
		}
	}
}
	...
	foreach ($arResult['PROPERTIES']['RAM']['VALUE'] as $key => $value) {
		...
		if($ar_res = $res->GetNext()) {
			$ar_res['PREVIEW_PICTURE'] = CFile::GetPath($ar_res['PREVIEW_PICTURE']);
		  	$price = CPrice::GetBasePrice($ar_res['ID']);
		  	$ar_res['PRICE_INFO'] = $price;
		  	$arResult['PROPERTIES']['RAM']['FULL_INFO'][] = $ar_res;
		}
	}
}
...
if($USER->IsAuthorized()) {
	...
	$arUser = $rsUser->Fetch();
	$arResult['REMEMBERED_ITEMS'] = unserialize($arUser['UF_REMEMBER']);
}

$sec = CIBlockSection::GetList(Array(), Array("ID" => $arResult["IBLOCK_SECTION_ID"]), false, Array("NAME"));
...
if(!empty($arResult['DETAIL_PICTURE']['SRC'])) {
    ...
    $arResult['DETAIL_PICTURE']['SRC'] = $arResizeFile['src'];
}
foreach ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $key => $img) {
	$imgs = CFile::GetFileArray($img);
	$arResizeFile = CFile::ResizeImageGet(
        $img,
        array('width' => $imgs['WIDTH'], 'height', $imgs['HEIGHT']),
        BX_RESIZE_IMAGE_PROPORTIONAL,
        true,
        $arWaterMark
    );

	$arResult['PROPERTIES']['MORE_PHOTO']['VALUE'][$key] = $arResizeFile;
}
if(!empty($arResult['PROPERTIES']['PROIZVODITEL']['VALUE'])) {
	$res = CIBlockElement::GetList( Array("SORT"=>"ASC"), Array('IBLOCK_ID' => 21, '=NAME' => $arResult['PROPERTIES']['PROIZVODITEL']['VALUE']));
	$arResult['PROPERTIES']['PROIZVODITEL']['VALUE'] = '';
	if($ar_res = $res->GetNext()) {
		if(!empty($ar_res['PREVIEW_PICTURE']))
			$arResult['PROPERTIES']['PROIZVODITEL']['VALUE'] = CFile::GetPath($ar_res['PREVIEW_PICTURE']);
	}
}
if($arResult["PROPERTIES"]["DISCOUNT"]["VALUE"] == 'Да') {
	$arResult["DISCOUNT"][] = [
		"ICON" 			=> "/images/skidka_angle.png",
		"BORDER_IMG" 	=> "/images/discount-border.jpg",
		"WIDTH"			=> "50%",
		"HEIGHT"		=> "60px",
		"STYLES"		=> [
			'position'  => 'relative',
...
if($arResult["PROPERTIES"]["SELLOUT"]["VALUE"] == 'Да') {
	if(count($arResult["DISCOUNT"]) > 0 && !is_int(count($arResult["DISCOUNT"]) / 2)) {
		$arResult["DISCOUNT"][] = [
			"ICON" 			=> "/images/akciia.png",
			"BORDER_IMG" 	=> "/images/sellout-border.jpg",
			"WIDTH"			=> "50%",
			"HEIGHT"		=> "45px",
			"STYLES"		=> [
...
if($arResult["PROPERTIES"]["KIT"]["VALUE"] == 'Да') {
	if(count($arResult["DISCOUNT"]) > 0 && !is_int(count($arResult["DISCOUNT"]) / 2)) {
		$arResult["DISCOUNT"][] = [
			"ICON" 			=> "/images/komplekt.png",
			"BORDER_IMG" 	=> "/images/kit-border.jpg",
			"WIDTH"			=> "50%",
			"HEIGHT"		=> "45px",
			"STYLES"		=> [
				'object-position'  => 'right',
		...
		$arResult["DISCOUNT"][] = [
			"ICON" 			=> "/images/komplekt_angle.png",
			"BORDER_IMG" 	=> "/images/kit-border.jpg",
			"WIDTH"			=> "50%",
			"HEIGHT"		=> "60px",
			"STYLES"		=> [
...
if($arResult["PROPERTIES"]["ORIGINAL"]["VALUE"] == 'Да') {
	if(count($arResult["DISCOUNT"]) > 0 && !is_int(count($arResult["DISCOUNT"]) / 2)) {
		$arResult["DISCOUNT"][] = [
			"ICON" 			=> "/images/original.png",
			"BORDER_IMG" 	=> "/images/origin-border.jpg",
			"WIDTH"			=> "50%",
			"HEIGHT"		=> "45px",
			"STYLES"		=> [
				'object-position'  => 'right',
		...
		$arResult["DISCOUNT"][] = [
			"ICON" 			=> "/images/original_angle.png",
			"BORDER_IMG" 	=> "/images/origin-border.jpg",
			"WIDTH"			=> "50%",
			"HEIGHT"		=> "60px",
			"STYLES"		=> [
...
if($arResult["PROPERTIES"]["HIGHTQUALITY"]["VALUE"] == 'Да') {
	$arResult["DISCOUNT"][] = [
		"ICON" 			=> "/images/detal_angle.png",
		"BORDER_IMG" 	=> "/images/quality-border.jpg",
		"HEIGHT"		=> "66px",
		"WIDTH"			=> "100%",
		"STYLES"		=> [
			'position'  => 'relative',
