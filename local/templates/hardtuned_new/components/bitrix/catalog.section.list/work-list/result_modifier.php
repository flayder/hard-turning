...
foreach ($arResult['SECTIONS'] as $key => $section) {
	$arResult['SECTIONS'][$key]['ELEMENTS'] = [];
	$res = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("SECTION_ID" => $section["ID"], "IBLOCK_ID" => $arResult["IBLOCK_ID"]), false, false, Array());

	while ($obElement = $res->GetNextElement()) {
		$el = $obElement->GetFields();
   		$el["PROPERTIES"] = $obElement->GetProperties();
		$arResult['SECTIONS'][$key]['ELEMENTS'][] = $el;
	}
}
?>